...
from myapp.form  import(
    Home_products,
    Upload_challan,Sign_up_form,Login_form,shop_form,verify_shop,forget_id_f,Order_form,My_form
)
...
def home_page(request):
    obj1=Home_page_products.objects.all()
    obj2=Main_products.objects.all()
    context={
    "object1":obj1,
    "object2":obj2
    }
        ...
        if form.is_valid():
            ...
            if platform=="Main_page":
                ...
                try:
                    ...
                    messages.success(request,"Sorry !!! Only one plan is allowed at one time ")    

            else:
                Home_page_products.objects.create(**form.cleaned_data)
...
def cart(request,id):
    obj=Home_page_products.objects.get(pk=id)   
    try:
        Cart.objects.create(
        id=obj.id,
        name=obj.name,
...
def cart_main(request,id,id2):
    obj=Main_products.objects.get(pk=id2)   
    try:
        Cart.objects.create(
        id=obj.id,
        name=obj.name,
...
def shop_cart(request,id,id2):
    ...
    try:
        ...
        return HttpResponse("<h1 align='center'>Product added to the card successfully !! </h1>")        
    except:            
        return HttpResponse("<h1 align='center'>The product is already added to the cart !!! </h1>")        
...
def admin_verification(request):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            query=f"select* from myapp_admin_verification where pasword= '{password}' "
...
def remove_from_cart(request,id):
    try:
        ...
        messages.success(request,"You have already removed the product from the cart !! ")
        my_url=reverse('check-cart')
        return HttpResponseRedirect(my_url)
...
def upload_challan(request):
    form=Upload_challan()
    if request.method=="POST":
        form=Upload_challan(request.POST,request.FILES)
        if form.is_valid():
            name=request.POST['email']
            check_list=[]
            ...
            if name in check_list:
                ...
                messages.success(request,"Sorry !! This email adress is not registered ")    
    context={
        "form":form
    }    
...
def reject_challan(request,id):
    try:
        ...
        obj.delete()
        return HttpResponse("<h1 align='center'>Challan is rejected sucsessfully !!</h1>")
    except:
        return HttpResponse("<h1 align='center'>The challan is already rejected !! </h1>")  
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.success(request,"The user name is already taken !! Please try with another  ")    
    context={
        "form":form
    }        
...
def allow_main(request,*args,**kwargs):
    ...
    return HttpResponse("<h1 align='center'>The product is uploaded to the main page !!!</h1>")
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            if email in check_list2:
                messages.success(request,"Only one shop is allowed at one email adress !!")
            else:    
                for i in variable1:
                ...
                if shop_id in check_list or shop_id in check_list1 :
                    messages.success(request,"The shop with this id is already approved !! ")    
                else:
                    try:
                        Shop.objects.create(**form.cleaned_data)
                        messages.success(request,"Your detail is send successfully !! We will verify your shop as soon as possible ")
                    except:
                        messages.success(request,"The Shop with this ID is already registerd !!")    
    context={
        "form":form
    }
...
def reject_shop_request(request,id):
    obj=Shop.objects.get(pk=id)
    obj.delete()
    return HttpResponse("<h1 align='center'>The Request has been rejected successfully !! </h1>")
def verify_shop_f(request):
    form=verify_shop()
    if request.method=="POST":
        form=verify_shop(request.POST)
        if form.is_valid():
            shop_id=request.POST['shop_id']
            email=request.POST['email']
            ...
            if variable is not None:
                ...
                return HttpResponse("<h1 align='center'>No Shop  with this data is registered !! </h1>")         
    context={
        "form":form
    }        
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            if variable is not None:
                ...
                try:
                    message.send()
                    messages.success(request,"The email is sent your verified email adress !! ")
                except:    
...
def main_products_detail(request,id):
    obj=Main_products.objects.get(pk=id)    
    context={
        "object":obj
    }
